...
import { Play, Pause, Wifi, Activity, Plus, Trash2, Zap, Database, XCircle, Search, Radio, MessageSquare, Scale, Info, TrendingDown, BrickWall } from 'lucide-react';
...
const doIntersect = (p1: {x:number, y:number}, q1: {x:number, y:number}, p2: {x:number, y:number}, q2: {x:number, y:number}) => {
  const onSegment = (p: any, r: any, q: any) => p.x <= Math.max(q.x, r.x) && p.x >= Math.min(q.x, r.x) && p.y <= Math.max(q.y, r.y) && p.y >= Math.min(q.y, r.y);
...
const App: React.FC = () => {
    ...
    let newVisuals = visualPacketsRef.current.map(vp => {
      ...
      if (vp.style === 'LINE') {
         ...
         for (const w of currentWalls) {
             if (doIntersect({x: vp.startX, y: vp.startY}, {x: vp.x, y: vp.y}, {x: w.x1, y: w.y1}, {x: w.x2, y: w.y2})) {
                 return null;
             }
         }
      }
    ...
    currentNodes.forEach(sender => {
      while(sender.txQueue.length > 0) {
        ...
        if (packet.destId === -1) {
            ...
            newVisuals.push({
                id: Math.random().toString(), packet: packet, x: sender.x, y: sender.y, startX: sender.x, startY: sender.y, targetId: -1, progress: 0, speed: speed, style: 'RING', maxRadius: rangePx
            });
        }
        ...
        currentNodes.forEach(receiver => {
          ...
          if (dist <= rangePx) {
             let blocked = false;
             for (const w of currentWalls) {
                 if (doIntersect({x: sender.x, y: sender.y}, {x: receiver.x, y: receiver.y}, {x: w.x1, y: w.y1}, {x: w.x2, y: w.y2})) {
                     blocked = true; break;
                 }
             }
             ...
             if (!blocked) {
                 ...
                 if (packet.destId !== -1) {
                     newVisuals.push({
                        id: Math.random().toString(), packet: packet, x: sender.x, y: sender.y, startX: sender.x, startY: sender.y, targetId: receiver.id, progress: 0, speed: 2.5, style: 'LINE'
                     });
                 }
             }
          }
  ...
  const nukeAll = () => { setNodes([]); nodesRef.current = []; setOpenWindows([]); setWalls([]); };
  const openNodeWindow = (id: number) => { if (!openWindows.includes(id)) { setOpenWindows(prev => [...prev, id]); setWindowOrder(prev => [...prev, id]); } else focusWindow(id); };
  ...
  const getPacketColor = (type: PacketType) => {
    switch (type) {
      case PacketType.HELLO: return '#38bdf8'; case PacketType.DATA: return '#4ade80'; case PacketType.ELECTION: return '#a855f7'; case PacketType.PANIC: return '#ef4444'; default: return '#cbd5e1';
    }
  };
  const styles = {
  ...
  return (
    ...
    <div style={styles.container} onMouseUp={handleMouseUp} onMouseMove={handleMouseMove} onMouseDown={(e) => handleMouseDown(e, 'bg')}>
      <div style={styles.sidebar}>
        <div><h1 style={{margin:0, color:'#38bdf8', fontSize:'22px', fontWeight:'900'}}>MESH SIM v21</h1><p style={{margin:0, color:'#64748b', fontSize:'11px'}}>Energy & Obstacle Physics</p></div>
        ...
        <div style={styles.panel}><span style={styles.label}>Tools</span><button style={{...styles.btn, backgroundColor: isDrawingWall ? '#a855f7' : '#334155'}} onClick={() => setIsDrawingWall(!isDrawingWall)}><BrickWall size={14} /> {isDrawingWall ? 'Finish Drawing' : 'Draw Walls'}</button><button style={{...styles.btn, backgroundColor: '#334155'}} onClick={() => setWalls([])}><Trash2 size={14} /> Clear Walls</button></div>
        
        <div style={styles.panel}><span style={styles.label}>Telemetry</span>
           <button style={{...styles.btn, backgroundColor: showCloudLogs ? '#3b82f6' : '#334155'}} onClick={() => setShowCloudLogs(!showCloudLogs)}><Database size={14} /> Cloud Database</button>
           <button style={{...styles.btn, backgroundColor: showPacketSniffer ? '#8b5cf6' : '#334155'}} onClick={() => setShowPacketSniffer(!showPacketSniffer)}><Radio size={14} /> Packet Sniffer</button>
           <button style={{...styles.btn, backgroundColor: showBatteryMonitor ? '#10b981' : '#334155'}} onClick={() => setShowBatteryMonitor(!showBatteryMonitor)}><TrendingDown size={14} /> Battery Monitor</button>
        </div>
        
        <div style={styles.panel}>
            <span style={styles.label}>Configuration</span>
            <input type="range" min="5" max="30" value={config.uwbRange} onChange={e => setConfig({...config, uwbRange: Number(e.target.value)})} style={{width:'100%'}} /><div style={{fontSize:'9px', color:'#cbd5e1', textAlign:'right'}}>{config.uwbRange}m</div>
            <input type="range" min="0.1" max="3.0" step="0.1" value={config.movingSpeed} onChange={e => setConfig({...config, movingSpeed: Number(e.target.value)})} style={{width:'100%'}} /><div style={{fontSize:'9px', color:'#cbd5e1', textAlign:'right'}}>{config.movingSpeed}x</div>
            <input type="range" min="1" max="5" step="1" value={config.maxLeaders} onChange={e => setConfig({...config, maxLeaders: Number(e.target.value)})} style={{width:'100%', accentColor:'#ec4899'}} /><div style={{fontSize:'9px', color:'#f472b6', textAlign:'right'}}>{config.maxLeaders} Leaders</div>
            
            {/* RESTORED SLIDER */}
            <input type="range" min="2" max="10" step="1" value={config.minClusterSize} onChange={e => setConfig({...config, minClusterSize: Number(e.target.value)})} style={{width:'100%', accentColor:'#a855f7'}} /><div style={{fontSize:'9px', color:'#a855f7', textAlign:'right'}}>Min Size: {config.minClusterSize}</div>
        </div>
      ...
      <div style={styles.main}>
        ...
        {showBatteryMonitor && (
           <DraggableWindow id="battery" title="SYSTEM BATTERY AVG" icon={TrendingDown} initialX={50} initialY={500} onClose={() => setShowBatteryMonitor(false)} onFocus={() => focusWindow('battery')} zIndex={100}>
             <div style={{padding:'10px', height:'150px'}}>
                <svg width="100%" height="100%" viewBox="0 0 50 100" preserveAspectRatio="none" style={{borderLeft:'1px solid #334155', borderBottom:'1px solid #334155'}}>
                    <polyline points={energyHistory.map((val, i) => `${i},${100-val}`).join(' ')} fill="none" stroke="#10b981" strokeWidth="2" />
                </svg>
             </div>
           </DraggableWindow>
        )}
